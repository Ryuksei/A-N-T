...
(function (h, f) {
  ...
  function pa() {
    ...
    f.ajax($ + '//m.agar.io/', {
      ...
      success: function (a) {
        a = a.split('\n');
        '37.187.163.131:1507' == a[0] ? pa()  : Ha('ws://' + a[0])
      },
      dataType: 'text',
      method: 'POST',
      cache: !1,
  ...
  function oa() {
    ...
    0 != I && (null == ia && (ia = new ja(24, '#FFFFFF', true,'#000000')), ia.u('Puntuación: ' + ~~(I / 100) + ' || Mejor puntuación: ' + ~~(sessionScore / 100) + " || Mejor tiempo vivo: " + bestTime + " seconds"), c = ia.G(), a = c.width, e.globalAlpha = 0.2, e.fillStyle = '#000000', e.fillRect(10, q - 10 - 24 - 10, a + 10, 34), e.globalAlpha = 1, e.drawImage(c, 15, q - 10 - 24 - 5));
    eb();
    b = Date.now() - b;
  ...
  function drawStats(d) {
    ...
    debugStrings.push("Servidor: " + serverIP);
    debugStrings.push("Sobrevivido por: " + nbSeconds + " seconds");

    if (getPlayer().length > 0) {
        debugStrings.push("Coordenadas: " + Math.floor(getPlayer()[0].x) + ", " + Math.floor(getPlayer()[0].y));
    }
